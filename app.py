# app.py (V4 - Adds SOE Fees Write Off tab + dual upload)
...
def parse_exam_status_report(uploaded_file) -> pd.DataFrame:
    """
    Parses the Exam Status sheet report (your existing logic).
    """
...
# -----------------------------
# NEW: SOE Students (for linking Validity Expired with write-off)
# -----------------------------
@st.cache_data(show_spinner=False)
def load_soe_students(student_profile_file) -> pd.DataFrame:
    """
    Reads 'SOE Students' sheet from Student Profile.xlsx.
    Must include Enrollment No + Exam Status.
    """
...
# -----------------------------
# NEW: Parse SOE Fees Write Off workbook (multi-sheet)
# -----------------------------
BVP_ENR_PATTERN = re.compile(r"\((BVP\d+)\)")

def _find_particulars_row(df: pd.DataFrame) -> int:
    # find row where any cell == "Particulars" (case-insensitive)
...
def _parse_sheet_writeoff(sheet_df: pd.DataFrame, sheet_name: str) -> pd.DataFrame:
    """
    Extracts rows between student lines and Grand Total.
    Expected layout similar to your screenshot: Particulars + Opening Balance + Debit + Credit + Closing Balance.
    """
    df = sheet_df.copy()
    ...
    # After "Particulars" row, there are header rows; data typically begins a few rows later.
    # We'll detect first data row where first column is a number (S.No) and Particulars is non-empty.
    # Identify columns by position: the row with "Particulars" contains it in some column (often col1).
    ...
    # Numeric columns are usually to the right of particulars col
    # We'll map: Opening Balance, Debit, Credit, Closing Balance by scanning header rows below.
    # Fallback: assume next 4 columns after particulars are the amounts.
    ...
    sno_col = 0  # commonly first column
    ...
    for i in range(pr + 1, len(df)):
        ...
        # student rows often have sno as 1.0, 2.0 ...
        # but some sheets may have blank; still accept if particulars has (BVP...)
    ...
    # Remove rows without Enrollment No (can’t link)
...
# Parse Exam Status report (for main dashboard)
try:
...
# Load SOE Students (for linking with write-off)
try:
...
with tab7:
    ...
    st.caption("Linked = students whose Exam Status is '13. VALIDITY EXPIRED' in SOE Students, matched by Enrollment No (BVP...)")

    # Breakdown by Programme & Batch
    ...
    st.dataframe(pb, use_container_width=True, height=450)

    st.caption("Top Programme-Batch by Closing Balance (Validity Expired only)")
    st.bar_chart(pb.set_index(pb["Programme"] + " | " + pb["Batch"])[["ClosingBalance"]].head(30))

    # Top students
    st.markdown("### Top Validity Expired Students by Closing Balance")
    topn = (
        linked.groupby(["Enrollment No", "Student Name", "Programme", "Batch"], dropna=False)[["ClosingBalance", "Debit", "Credit"]]
        .sum()
        .reset_index()
        .sort_values("ClosingBalance", ascending=False)
        .head(50)
    )
    st.dataframe(topn, use_container_width=True, height=500)

    # Optional: show raw linked rows
    with st.expander("Show linked raw rows"):
        st.dataframe(linked.sort_values(["Programme", "Batch", "ClosingBalance"], ascending=[True, True, False]),

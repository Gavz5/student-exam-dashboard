# app.py (V5 - Adds Vendor-wise + Program-Batch-Vendor summaries for SOE Fees Write Off)
...
# -----------------------------
# SOE Fees Write Off workbook parser (+ Vendor + SheetProgram/SheetBatch)
# -----------------------------
BVP_ENR_PATTERN = re.compile(r"\((BVP\d+)\)")
SHEET_META_PATTERN = re.compile(
    r"^\s*(?P<prog>.+?)\s*\(\s*(?P<batch>.+?)\s*\)\s*$"
)
...
def _detect_vendor(sheet_df: pd.DataFrame, sheet_name: str) -> str:
    """
    Vendor detection priority:
    1) If top rows contain a cell 'Vendor'/'Vendor Name' -> take adjacent cell
    2) Else Vendor = sheet name (safe fallback)
    """
    df = sheet_df.copy()
    ...
    for i in range(scan_rows):
        for j in range(scan_cols):
            ...
            if pd.isna(v):
                continue
            s = str(v).strip().lower()
            if s in {"vendor", "vendor name", "vendor:", "vendor name:"}:
                # try right cell
                if j + 1 < df.shape[1]:
                    right = df.iat[i, j + 1]
...
def _sheet_program_batch(sheet_name: str):
    """
    Best-effort parse from sheet name:
    'MBA-II (Jan 22)' => SheetProgram='MBA-II', SheetBatch='Jan 22'
    If not match, return (None, None)
    """
    m = SHEET_META_PATTERN.match(str(sheet_name))
    if not m:
        return None, None
    return m.group("prog").strip(), m.group("batch").strip()
...
def _parse_sheet_writeoff(sheet_df: pd.DataFrame, sheet_name: str) -> pd.DataFrame:
    ...
    vendor = _detect_vendor(df, sheet_name)
    sheet_prog, sheet_batch = _sheet_program_batch(sheet_name)
        ...
        records.append(
            {
                "Sheet": sheet_name,
                "Vendor": vendor,
                "SheetProgram": sheet_prog,
                "SheetBatch": sheet_batch,
                "Enrollment No": enr,
...
with tab7:
    ...
    st.caption("Linked = Exam Status '13. VALIDITY EXPIRED' in SOE Students matched by Enrollment No (BVP...)")

    # A) Programme-wise / Batch-wise (from SOE Students)
    st.markdown("### A) Validity Expired: Programme-wise / Batch-wise")
    pb = (
    ...
    # B) Vendor-wise
    st.markdown("### B) Validity Expired: Vendor-wise")
    vw = (
    ...
    # C) Program-Batch-Vendor (what you asked)
    st.markdown("### C) Validity Expired: Programme + Batch + Vendor (Program-wise + Batch-wise + Vendor-wise)")
    pbv = (
    ...
    st.dataframe(pbv, use_container_width=True, height=450)

    # D) Sheet-derived Program/Batch (if you want that view also)
    st.markdown("### D) Sheet-based grouping (SheetProgram / SheetBatch / Vendor) [optional]")
    sbv = (
        linked.groupby(["SheetProgram", "SheetBatch", "Vendor"], dropna=False)[["ClosingBalance", "Debit", "Credit"]]
        .sum()
        .reset_index()
        .sort_values("ClosingBalance", ascending=False)
    )
    st.dataframe(sbv, use_container_width=True, height=350)
